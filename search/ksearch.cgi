...
sub returnresults {	# creates HTML page from template file
	...
	if ($query->param('help') == 1) {
		...
		# $commonterms = $commonterms.' (see <a href="'.$SEARCH_URL.'?'.$FORM_INPUT_NAME.'='.$query_str.'&amp;help=1&amp;display='.$RESULTS_PER_PAGE.$sortby.$options.'" alt="Search Tips"><strong>tips</strong></a>)' if $commonterms;
		$h{subheader} = $commonterms.$search_sources.$weight_tip;
	...
	foreach ((@sortedanswers)[$first..$last]) {
		...
		if ($SORT_BY eq "Matches") {
			$scoreA = '<strong>Matches:</strong> '.$matches{$_};
			$scoreB = "<strong>$score</strong> ".$finalscores{$_};
			$scoreC = '<strong>Last Updated:</strong> '.get_date($f_date_db{$_});
			$scoreD = '<strong>File Size:</strong> '.$f_size_db{$_}.'KB';
		} elsif ($SORT_BY eq "Scores") {
			$scoreA = "<strong>$score</strong> ".$finalscores{$_};
			$scoreB = '<strong>Matches:</strong> '.$matches{$_};
			$scoreC = '<strong>Last Updated:</strong> '.get_date($f_date_db{$_});
			$scoreD = '<strong>File Size:</strong> '.$f_size_db{$_}.'KB';
		} elsif ($SORT_BY eq "Dates")  {
			$scoreA = '<strong>Last Updated:</strong> '.get_date($f_date_db{$_});
			$scoreB = '<strong>Matches:</strong> '.$matches{$_};
			$scoreC = "<strong>$score</strong> ".$finalscores{$_};
			$scoreD = '<strong>File Size:</strong> '.$f_size_db{$_}.'KB';
		} else {
			$scoreA = '<strong>Matches:</strong> '.$matches{$_};
			$scoreB = '<strong>File Size:</strong> '.$f_size_db{$_}.'KB';
			$scoreC = "<strong>$score</strong> ".$finalscores{$_};
			$scoreD = '<strong>Last Updated:</strong> '.get_date($f_date_db{$_});
		}
		...
		if ($PDF_TO_TEXT && $PDF_INFO && $f_file_db{$_} =~ m/\.pdf$/i) {
			$title = $title.' &nbsp; - &nbsp; <strong><em>PDF '.$f_size_db{$_}.'KB</em></strong>';
		}
